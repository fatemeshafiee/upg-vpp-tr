  ...
  shdefault(usage_hash, NULL);
    for(u32 i=0; i < num; i++){
      ...
      if (flow->stats[0].pkts!=0 || flow->stats[1].pkts!=0){
        ...
        usage_report_per_flow_t* usage_report_per_flow_vector = shgetp(usage_hash, new_data->src_ip);
        ...
        shput(usage_hash,strdup(new_data->src_ip),usage_report_per_flow_vector);
